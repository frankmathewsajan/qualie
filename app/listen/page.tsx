...
import {
  ...
  MapPin, AlertTriangle, Navigation, Thermometer, ChevronRight, Settings, EyeOff, BellRing, Phone, Copy
} from 'lucide-react';
...
export default function ListenPage() {
  ...
  const [sosOptions, setSosOptions] = useState<{ phone: string; text: string } | null>(null);
  ...
  const triggerWhatsAppSOS = useCallback(async () => {
    try {
      ...
      const msg = `AEGIS EMERGENCY ALERT\n\n` +
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans select-none overflow-x-hidden transition-colors duration-1000"
      ...
      {/* ── SOS Options Overlay ────────────────────────────────────────── */}
      {sosOptions && (
        <div className="fixed inset-0 z-[100000] bg-black/60 flex items-end justify-center p-4" onClick={() => setSosOptions(null)}>
          <div className="w-full max-w-sm bg-white rounded-3xl p-6 text-center shadow-2xl mb-8" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-slate-500 mb-6">How would you like to send your emergency alert?</p>
            ...
            <div className="flex flex-col gap-3">
              <button onClick={() => {
                ...
                setSosOptions(null);
              }} className="w-full py-4 bg-[#25D366] hover:bg-[#20bd5a] text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-colors">
              ...
              <button onClick={() => {
                navigator.clipboard.writeText(sosOptions.text);
                setSosOptions(null);
                alert("Emergency text copied to clipboard!");
              }} className="w-full py-4 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold flex items-center justify-center gap-2 transition-colors">
                <Copy className="w-5 h-5" />
                Copy Text Only
              </button>
              <button onClick={() => setSosOptions(null)} className="w-full py-3 text-slate-400 hover:text-slate-600 font-medium mt-2">
                Cancel
              </button>
            </div>
          </div>
